...
if __name__ == "__main__":
    ...
    for i, seed in enumerate(np.random.choice(1000, size=(100,), replace=False)):
        ...
        s_stats = [0, np.sum(s_records["goal"]), np.sum(s_records["collision"]),
                   np.sum(np.multiply(s_records["goal"], s_records["distance"])) / (np.sum(s_records["goal"]) + 1e-16),
        ...
        d_stats = [0, np.sum(d_records["goal"]), np.sum(d_records["collision"]),
                   np.sum(np.multiply(d_records["goal"], d_records["distance"])) / (np.sum(d_records["goal"]) + 1e-16),
