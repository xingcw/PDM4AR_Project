...
class MPCController(object):
    def __init__(self, path, sg: SpacecraftGeometry, x0: SpacecraftState):
        ...
        mterm = (self.model.x['x'] - self.model.tvp['x_gt']) ** 2 + (
                    self.model.x['y'] - self.model.tvp['y_gt']) ** 2 + 3 * (
                            self.model.x['psi'] - self.model.tvp['psi_gt']) ** 2

        lterm = (self.model.x['x'] - self.model.tvp['x_gt']) ** 2 + (
                    self.model.x['y'] - self.model.tvp['y_gt']) ** 2 + 3 * (
                            self.model.x['psi'] - self.model.tvp['psi_gt']) ** 2
    ...
    def update_planned_target(self, plan_sequence):
        ...
        def tvp_fun(t_now):
            for k in range(self.n_horizon):
                ...
                tvp_template['_tvp', k, 'psi_gt'] = plan_sequence[k, 2]
            return tvp_template
